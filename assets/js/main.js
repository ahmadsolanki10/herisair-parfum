(function(){
 ...
 const contact=$('[data-contact]');if(contact)contact.onsubmit=async e=>{e.preventDefault();const note=$('.form-note',contact),btn=$('button',contact);if(!contact.checkValidity()){contact.reportValidity();return}btn.disabled=true;btn.textContent='Sending…';contact.setAttribute('aria-busy','true');try{const endpoint=contact.dataset.endpoint;if(!endpoint)throw Error('Client Care is temporarily unavailable. Please email info@herisair.com');const res=await fetch(endpoint,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(Object.fromEntries(new FormData(contact)))});const data=await res.json().catch(()=>({}));if(!res.ok)throw Error(data.error||'Your enquiry could not be sent. Please email info@herisair.com');note.textContent='Thank you. Your enquiry has reached Hérisair Client Care.';contact.reset()}catch(error){note.textContent=error.message||'Your enquiry could not be sent. Please email info@herisair.com'}finally{btn.disabled=false;btn.textContent='Send enquiry';contact.removeAttribute('aria-busy')}};
 const toast=$('.toast');
